...
void VarDA::AdjointMatAssySNS(const int ic, MatrixXd &Klocal, VectorXd &Flocal, VDOUBLE2D &externalForce)
{
  int ii, jj;
  int IU,IV,IW,IP;
  int JU,JV,JW,JP;
...
void VarDA::AdjointBdMatAssySNS(const int ic, MatrixXd &Klocal, VectorXd &Flocal, VDOUBLE2D &externalForce)
{
  int ii, jj;
  int IU,IV,IW,IP;
  int JU,JV,JW,JP;
...
void VarDA::calcBoundaryIntegral(const int ic, MatrixXd &Klocal, VectorXd &Flocal)
{
  int ii, jj;
  int IU,IV,IW,IP,IL,IM,IN;
  int JU,JV,JW,JP,JL,JM,JN;

  int GP = 2;
  double detJ, weight;
  Gauss gauss(GP);

  VDOUBLE2D x_current2D(numOfNodeInElm, VDOUBLE1D(2,0e0));
  ...
  for(int i1=0; i1<GP; i1++){
    for(int i2=0; i2<GP; i2++){
      ...
      MathFEM::calc_dxdr2D(dxdr2D, dNdr2D, x_current2D, 4);
      
      detJ = dxdr2D[0][0] * dxdr2D[1][1] - dxdr2D[0][1] * dxdr2D[1][0];
      ...
      {  
        ...
        {  
          ...
          Klocal(IL, JL) += N2D[ii] * N2D[jj] * detJ * weight; 
          Klocal(IM, JM) += N2D[ii] * N2D[jj] * detJ * weight; 
          Klocal(IN, JN) += N2D[ii] * N2D[jj] * detJ * weight; 
        }
      }
