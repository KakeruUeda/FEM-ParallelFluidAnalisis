  ...
  for(int i1=0; i1<GP; i1++){
    for(int i2=0; i2<GP; i2++){
      for(int i3=0; i3<GP; i3++){
        ...
        ShapeFunction3D::C3D8_N(NP,gauss.point[i1],gauss.point[i2],gauss.point[i3]);
        ShapeFunction3D::C3D8_N(NV,gauss.point[i1],gauss.point[i2],gauss.point[i3]);
        ShapeFunction3D::C3D8_dNdr(dNPdr,gauss.point[i1],gauss.point[i2],gauss.point[i3]);
        ShapeFunction3D::C3D8_dNdr(dNVdr,gauss.point[i1],gauss.point[i2],gauss.point[i3]); 
        ...
        if(sdfFluid[ic]<=0){
          ...
          if(sdfFluid[ic]/minSDF<1.0){
            for(int p=0;p<numOfNodeInElm;p++){
              for(int i=0;i<3;i++) dNVdr[p][i] = dNPdr[p][i] * sdfFluid[ic]/minSDF;
              NV[p] = NP[p] * sdfFluid[ic]/minSDF;       
            }
          }
        }
...
void FEM::XFEM_MatAssySTT3(const int ic, MatrixXd &Klocal, VectorXd &Flocal)
{
  int ii, jj;
  int IU, IV, IW, IP;
  int JU, JV, JW, JP;
