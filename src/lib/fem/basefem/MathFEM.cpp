...
void MathFEM::calc_dxdr2D(double (&dxdr)[2][2], VDOUBLE2D &dNdr, VDOUBLE2D &x1, const int &numOfNodeInElm)
{
  for(int i=0;i<2;i++){
    for(int j=0;j<2;j++){
      dxdr[i][j] = 0e0;
...
void MathFEM::calc_dNdx2D(VDOUBLE2D &dNdx, VDOUBLE2D &dNdr, const double (&dxdr)[2][2], const int &numOfNodeInElm)
{
  double drdx[2][2];
  BasicFunctions::calcInverseMatrix_2x2(drdx, dxdr);
